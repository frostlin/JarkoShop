...
public class ToProfileCommand implements Command {
    ...
    public String execute(HttpServletRequest request) {
        ...
        if (session.getAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PAGE) == null)
            session.setAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PAGE, 1);
        if (session.getAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PER_PAGE) == null)
            session.setAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PER_PAGE, PaginationConstants.CURRENT_PROFILE_ORDERS_PER_PAGE);

        int pageNumber = (int)session.getAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PAGE);
        int itemsPerPage = (int)session.getAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PER_PAGE);

        String nextPageNumber = request.getParameter(RequestParameter.NEXT_ITEM_PAGE);
        if (nextPageNumber != null){
            pageNumber = Integer.parseInt(nextPageNumber);
            session.setAttribute(SessionAttribute.CURRENT_PROFILE_ORDERS_PAGE, pageNumber);
        }
        try {
            int orderCount = orderService.getActiveOrderCountByUser(((User)(session.getAttribute(SessionAttribute.CURRENT_USER))).getId());
            int itemPageCount = (int) Math.ceil(orderCount * 1.0 / itemsPerPage);
            List<Order> range = orderService.getRangeActive(pageNumber, itemsPerPage);
